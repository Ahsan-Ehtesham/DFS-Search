    ...
    if placeNodes == True:
        if nodes:  # Checks if the nodes are not empty
            for p, location in enumerate(nodes):
                if (
                    pos[0] >= (nodes[p].pos[0] - NODE_SPACE_ALLOWANCE)
                    and pos[0] <= (nodes[p].pos[0] + NODE_SPACE_ALLOWANCE)
                ) and (
                    pos[1] >= (nodes[p].pos[1] - NODE_SPACE_ALLOWANCE)
                    and pos[1] <= (nodes[p].pos[1] + NODE_SPACE_ALLOWANCE)
                ):
        ...
        for n, point in enumerate(nodes):
            if nodes[n].children:
                for child in nodes[n].children:
                    canvas.draw_line(
                        nodes[n].pos, nodes[child].pos, EDGE_SIZE, EDGE_COLOR
                    )
...
button1 = frame.add_button("Set Nodes", button_lock_nodes)
label1 = frame.add_label(" ")

button2 = frame.add_button("Set Graph", button_lock_graph)
label2 = frame.add_label(" ")

# button3 = frame.add_button('Reset edge drawing', button_refresh_new_relation)
# label3 = frame.add_label(' ')

input_start = frame.add_input("Start Node", input_start_handler, 50)
label4 = frame.add_label(" ")

input_goal = frame.add_input("Goal Node", input_goal_handler, 50)
